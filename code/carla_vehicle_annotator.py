...
def auto_annotate_lidar(vehicles, camera, carla_img, lidar_data, seg_data, file_name = '', max_dist=100, min_detect=5, show_img=None,
                        json_path=None):
    filtered_data = filter_lidar(lidar_data, camera, max_dist)
    if show_img != None:
        show_lidar(filtered_data, camera, show_img, file_name)
    ...
    bounding_boxes_2d_old = [get_2d_bb(vehicle, camera)[1] for vehicle in visible_vehicles]
    bounding_boxes_3d = [get_2d_bb(vehicle, camera)[0] for vehicle in visible_vehicles]
    # print("old:", bounding_boxes_2d)
    vehicle_color = np.array([142, 0, 0])
    height, width = int(camera.attributes['image_size_y']), int(camera.attributes['image_size_x'])
    try:
        bounding_boxes_2d = seg_fitting(bounding_boxes_2d_old, seg_data, vehicle_color, width, height)
    except:
        print(bounding_boxes_2d_old, carla_img.frame, width, height)
        for bounding_box in bounding_boxes_2d_old:
            point_1 = bounding_box[0]
            point_2 = bounding_box[1]
            x_1, x_2 = point_1[0], point_2[0]
            y_1, y_2 = point_1[1], point_2[1]

            max_x = int(max([x_1, x_2]))
            min_x = int(min([x_1, x_2]))
            max_y = int(max([y_1, y_2]))
            min_y = int(min([y_1, y_2]))
            print("original:", min_x, max_x, min_y, max_y)
            print("cut_off:", cut_off(min_x, width), cut_off(max_x, width), cut_off(min_y, height),
                  cut_off(max_y, height))
        bounding_boxes_2d = bounding_boxes_2d_old
    # print("new:",bounding_boxes_2d)
    filtered_out = {}
    filtered_out['vehicles'] = visible_vehicles
    filtered_out['bbox'] = bounding_boxes_2d
    filtered_out['old_bbox'] = bounding_boxes_2d_old
    filtered_out['3d_bbox'] = bounding_boxes_3d
...
def fitting_y(y1, y2, range_min, range_max, seg_info, color, width, height):
    # global seg_info
    state = False
    cali_point = 0
    y1, y2 = cut_off(y1, height), cut_off(y2, height)
    range_min, range_max = cut_off(range_min, width), cut_off(range_max, width)
    # if y1>=height:
    #     y1=height-1
    # else:
    #     y1=y1
...
def fitting_x(x1, x2, range_min, range_max, seg_info, color, width, height):
    # global seg_info
    state = False
    cali_point = 0
    x1, x2 = cut_off(x1, width), cut_off(x2, width)
    range_min, range_max = cut_off(range_min, height), cut_off(range_max, height)
    # if x1>=width:
    #     x1=width-1
    # else:
    #     x1=x1
...
def seg_fitting(bounding_boxes, seg_info, color, width, height):
    new_bounding_boxes = []
    for bounding_box in bounding_boxes:
        ...
        # print(cut_off(min_x, width), cut_off(max_x, width), cut_off(min_y, height), cut_off(max_y, height))
        cali_min_x = fitting_x(min_x, max_x, min_y, max_y, seg_info, color, width, height)
        cali_max_x = fitting_x(max_x, min_x, min_y, max_y, seg_info, color, width, height)
        cali_min_y = fitting_y(min_y, max_y, min_x, max_x, seg_info, color, width, height)
        cali_max_y = fitting_y(max_y, min_y, min_x, max_x, seg_info, color, width, height)
        v_cali_array = np.array([[cali_min_x, cali_min_y], [cali_max_x, cali_max_y]])
        new_bounding_boxes.append(v_cali_array)
...
def show_lidar(lidar_data, camera, carla_img, file_name):
    ...
    filename = 'yolo_out_lidar_img/{}.jpg'.format(file_name)
...
def save_output(carla_img, seg_img, bboxes, original_bboxes, three_d_bboxes, vehicle_class=None, old_bboxes=None,
                old_vehicle_class=None, cc_rgb=carla.ColorConverter.Raw, path='', file_name='', save_patched=False, add_data=None,
                out_format='pickle'):
    carla_img.save_to_disk(path + 'out_rgb/{}.png'.format(file_name), cc_rgb)
    # seg_img.convert(cc.CityScapesPalette)
    i = np.array(seg_img.raw_data)
    i2 = i.reshape((carla_img.height, carla_img.width, 4))
    i3 = i2[:, :, :3]
    cv2.imwrite(path + 'out_seg/{}.png'.format(file_name), i3)
    out_dict = {}
    bboxes_list = [bbox.tolist() for bbox in bboxes]
    original_bboxes_list = [bbox.tolist() for bbox in original_bboxes]
    three_d_bboxes_list = [bbox.tolist() for bbox in three_d_bboxes]
    # bboxes_list = [bbox for bbox in bboxes]
    out_dict['bboxes'] = bboxes_list
    out_dict['original_bboxes'] = original_bboxes_list
    out_dict['3d_bboxes'] = three_d_bboxes_list
    ...
    if out_format == 'json':
        filename = path + 'out_bbox/{}.txt'.format(file_name)
        if not os.path.exists(os.path.dirname(filename)):
            os.makedirs(os.path.dirname(filename))
        with open(filename, 'w') as outfile:
            json.dump(out_dict, outfile, indent=4)
    else:
        filename = path + 'out_bbox/{}.pkl'.format(file_name)
        if not os.path.exists(os.path.dirname(filename)):
            os.makedirs(os.path.dirname(filename))
        with open(filename, 'w') as outfile:
            json.dump(out_dict, outfile, indent=4)
    ...
    if save_patched:
        ...
        for crop in three_d_bboxes:
            # for crop in boxes:
            for i in [0, 4]:
                img_draw.line(
                    (int(crop[i, :2].tolist()[0][0]), int(crop[i, :2].tolist()[0][1]),
                     int(crop[i + 1, :2].tolist()[0][0]), int(crop[i + 1, :2].tolist()[0][1])),
                    fill='blue', width=2)
                img_draw.line(
                    (int(crop[i + 1, :2].tolist()[0][0]), int(crop[i + 1, :2].tolist()[0][1]),
                     int(crop[i + 2, :2].tolist()[0][0]), int(crop[i + 2, :2].tolist()[0][1])),
                    fill='blue', width=2)
                img_draw.line(
                    (int(crop[i, :2].tolist()[0][0]), int(crop[i, :2].tolist()[0][1]),
                     int(crop[i + 3, :2].tolist()[0][0]), int(crop[i + 3, :2].tolist()[0][1])),
                    fill='blue', width=2)
                img_draw.line(
                    (int(crop[i + 2, :2].tolist()[0][0]), int(crop[i + 2, :2].tolist()[0][1]),
                     int(crop[i + 3, :2].tolist()[0][0]), int(crop[i + 3, :2].tolist()[0][1])),
            ...
            for i in [0, 1, 2, 3]:
                img_draw.line(
                    (int(crop[i, :2].tolist()[0][0]), int(crop[i, :2].tolist()[0][1]),
                     int(crop[i + 4, :2].tolist()[0][0]), int(crop[i + 4, :2].tolist()[0][1])),
                    fill='blue', width=2)

        filename = path + 'out_rgb_bbox/{}.png'.format(file_name)
        if not os.path.exists(os.path.dirname(filename)):
            os.makedirs(os.path.dirname(filename))
        image.save(filename)
...
def save2darknet(bboxes, vehicle_class, carla_img, data_path='', cc_rgb=carla.ColorConverter.Raw, save_train=False,
    ...
    if bbr or vcr or cir:
        ...
        img_rgb = np.uint8(img_rgb)
        image = Image.fromarray(img_rgb, 'RGB')
        # os.makedirs(os.path.dirname(obj_path + '/%06d.jpg' % file_name))
        image.save(obj_path + '/' + str(customName) + '{}.jpg'.format(file_name))
        ...
        with open(obj_path + '/' + str(customName) + '{}.txt'.format(file_name), 'w') as filetxt:
